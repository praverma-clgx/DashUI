...
class WorkFlowBuilderPage {
    ...
    if (!result.success) {
      ...
      for (let attempt = 1; attempt <= 5; attempt++) {
        try {
          ...
          const dropdownContainer = addWorkModalFrame.locator(
            '#DropDown_AssignmentDelay_DropDown .rcbList',
          );
          await dropdownContainer.waitFor({ state: 'visible', timeout: 5000 });
          await this.page.waitForTimeout(800);

          const assignmentDelayUnitOptions = addWorkModalFrame.locator(
            '#DropDown_AssignmentDelay_DropDown .rcbList .rcbItem',
          );
    ...
    if (!result.success) {
      ...
      for (let attempt = 1; attempt <= 5; attempt++) {
        try {
          ...
          const dropdownContainer = addWorkModalFrame.locator(
            '#DropDown_CompleteWithin_DropDown .rcbList',
          );
          await dropdownContainer.waitFor({ state: 'visible', timeout: 5000 });
          await this.page.waitForTimeout(800);

          const mustCompleteWithinUnitOptions = addWorkModalFrame.locator(
            '#DropDown_CompleteWithin_DropDown .rcbList .rcbItem',
          );
  ...
  async selectRequiredCompletionAction(addWorkModalFrame) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      const result = await actualFrame.evaluate(
        ({ dropdownId, valueToSelect }) => {
          const comboBox = window.$find ? window.$find(dropdownId) : null;
          if (comboBox && typeof comboBox.findItemByText === 'function') {
            const item = comboBox.findItemByText(valueToSelect);
    ...
    if (!apiSuccess) {
      ...
      for (let attempt = 1; attempt <= 5; attempt++) {
        try {
          ...
          await addWorkModalFrame
            .locator('#DropDown_RequiredCompletionAction_DropDown .rcbList')
            .waitFor({ state: 'visible', timeout: 5000 });
          await this.page.waitForTimeout(800);
          ...
          if (attempt === 5)
            throw new Error(`Failed to select "${valueToSelect}" after 5 attempts: ${e.message}`);
          await this.page.keyboard.press('Escape').catch(() => {});
          await this.page.waitForTimeout(500);
        }
      }
    ...
    if (!result.success) {
      ...
      for (let attempt = 1; attempt <= 5; attempt++) {
        try {
          ...
          const dropdownContainer = addWorkModalFrame.locator(
            '#DropDown_AssociatedCompletionDate_DropDown .rcbList',
          );
          await dropdownContainer.waitFor({ state: 'visible', timeout: 5000 });
          await this.page.waitForTimeout(800);

          const associatedCompletionDateOptions = addWorkModalFrame.locator(
            '#DropDown_AssociatedCompletionDate_DropDown .rcbList .rcbItem',
          );
